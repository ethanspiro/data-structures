...
/* Name: Ethan Spiro
 * Date: 6/3/14
 * Solution description: Implementation of graph, DFS, BFS
 */
...
{
	/* USE cirListDeque as a STACK */

	/* Create a reachable set of vertices, initially empty */
	/* This is just the field isVisted */

	/*make sure all the vertex have been set to not visited */
	clearVisited(g);

	/*create a container for vertices known to be reachable, call this c */
	struct cirListDeque *c = malloc(sizeof(struct cirListDeque));
	initCirListDeque(c);

	/*add start vertex to container c */
	addBackCirListDeque(c, source);

	/*while the container c is not empty */
	while (!isEmptyCirListDeque(c)) {

		/*remove top entry from container, assign to v (mark as visited below) */
		Vertex *currentVertex = (backCirListDeque(c));
		...
		/*if v is not already flagged as reachable */
		if (!currentVertex->isVisited) {
			/* add v to the reachable set r */
			currentVertex->isVisited = 1;
			/* add neighbors of v to the container c */
			int i;
			for (i = 0; i < currentVertex->numNeighbors; i++) {
				addBackCirListDeque(c, currentVertex->neighbors[i]);
			}
		} /* end if */
	} /*end while */

	/*if you cant find a path return false */
	return 0;
}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	/*USE cirListDeque as a QUEUE */

	/* Create a reachable set of vertices, initially empty */
	/* This is just the field isVisted */

	/*make sure all the vertex have been set to not visisted */
	clearVisited(g);
	...
	while (!isEmptyCirListDeque(c)) {
		...
		/*remove first entry from queue, assign to v (mark as visited below) */
		Vertex *currentVertex = (frontCirListDeque(c));
