/* CS261- Assignment 5 - dynamicArray.c*/
/* Name: Ethan Spiro
 * Date: 5/5/14
 * Solution description: Adding heap implementation using a dynamic array
 */
...
int _smallerIndexHeap(DynArr *heap, int i, int j)
{
	//make sure i and j are smaller than size
...
{
    //make sure heap is not null
    assert(heap != NULL);

    //define an int to keep track of the parent for percolation
    int parent;
    //next available space is the size
    int pos = sizeDynArr(heap);

    //add val to the end of the heap
    addDynArr(heap, val);

    //percolate up if value at pos is less than value at parent
    while (pos != 0) {
    	//algorithm for accessing parent of the node
    	parent = (pos-1)/2;
    	//if the parent is less than the current position
    	...
    	//if the value is not less, than you dont percolate
    	else {
    		return;
    	}
    }
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	//make sure max (the index to adjust up to) is less than the size of the heap
	assert (max <= sizeDynArr(heap));

	//define two ints for children of the node
	int left = (2*pos)+1;
	int right = (2*pos)+2;

	//define an int for the smaller value index of the two
	int smallest;

	//2 children case
	if (right < max) {
		//find the smaller of the two index
		smallest = _smallerIndexHeap(heap, left, right);
		//if the child is less than the parent, swap and adjust again (recursive)
...
void sortHeap(DynArr *heap)
{
   //make sure the heap is not empty
	...
	int i;
	//sorting algorithm (swap first and last and then adjust)
