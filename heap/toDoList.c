...
/* Name: Ethan Spiro
 * Date: 5/5/14
 * Solution description: Implementation of a ToDo list using a heap
 */
...
void printList(DynArr *heap)
{
    ...
    //create a temp dyn array for printing purposes
    DynArr *printerList = createDynArr(sizeDynArr(heap));

    //copy the heap into the printerList
    copyDynArr(heap, printerList);

    //printing loop
    while (sizeDynArr(printerList) > 0) {
    	TYPE elementToPrint = getMinHeap(printerList);
    	print_type(elementToPrint);
    	removeMinHeap(printerList);
    }

    //free
    deleteDynArr(printerList);
}
...
{
    //cast left and right to corresponding pointer type
	Task* leftT = (Task*)left;
	Task* rightT = (Task*)right;

	//compare values using casted pointers
