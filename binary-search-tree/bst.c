...
/* Name: Ethan Spiro
 * Date: 5/5/14
 * Solution description: Implementation of binary search tree data structure
 */
...
{
	struct Node *newNode;
	int x;	//for use in compare conditionals

	//if the current node is null
	if(cur == NULL) {
		//allocate space for a new node
		newNode = (struct Node *)malloc(sizeof(struct Node));
		//make sure its been created
		assert (newNode != NULL);
		//fill with value
		newNode->val = val;
	...
	else {	//if the current root node is not null we keep moving down tree
		...
		else {	//else we move down to the right for insertion
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
	...
	while(current != NULL) {
		...
		if (x == 0) {
			...
			//if compare returns -1, move left
			if(x == -1) {
				current = current->left;
			}
			//if compare returns 1, move right
			else {
	...
	//return false if you traverse entire tree and the value hasnt been found
	return 0;
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	if (cur->left != NULL) {
		//move down left recursively
		cur->left = _removeLeftMost(cur->left);
		return cur;
	}

	//oterwise return right child of cur
	tempNode = cur->right;
	free(cur);
	return tempNode;
}
...
void testAddNode() {
    ...
    struct data myData1,  myData2,  myData3,  myData4; //CHANGED HERE removed myData4
